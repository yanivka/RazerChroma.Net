...
namespace RazerChromaFrameEngine
{
    public class KeypadFrame
    {
        ...
        private RazerChroma.Net.KeyPad.Effects.Custom rawEffect;
        private Effect lastEffect;

        public KeypadFrame(NativeRazerApi api)
        {
            this._api = api;
            this.rawEffect = new RazerChroma.Net.KeyPad.Effects.Custom(new NativeWin32.ColorRef[RazerChroma.Net.KeyPad.Definitions.MaxRow, RazerChroma.Net.KeyPad.Definitions.MaxCol]);
            this.lastEffect = null;
            for (int row = 0; row < RazerChroma.Net.KeyPad.Definitions.MaxRow; row++)
            {
                for (int col = 0; col < RazerChroma.Net.KeyPad.Definitions.MaxCol; col++)
                {
        ...
        {
            SetKeys(0, 0, (int)RazerChroma.Net.KeyPad.Definitions.MaxRow - 1, (int)RazerChroma.Net.KeyPad.Definitions.MaxCol - 1, color);
        }
        ...
        public void Update()
        {
            Effect newEffect = _api.CreateKeypadEffect(rawEffect);
            newEffect.Set();
            lastEffect?.Delete();
