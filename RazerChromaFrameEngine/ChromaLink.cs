...
{
    public class ChromaLinkFrame
    {

        private NativeRazerApi _api;
        private RazerChroma.Net.ChromaLink.Effects.Custom rawEffect;
        private Effect lastEffect;

        public ChromaLinkFrame(NativeRazerApi api)
        {
            this._api = api;
            this.rawEffect = new RazerChroma.Net.ChromaLink.Effects.Custom(new NativeWin32.ColorRef[RazerChroma.Net.ChromaLink.Definitions.MaxLeds]);
            this.lastEffect = null;
            for (int i = 0; i < RazerChroma.Net.ChromaLink.Definitions.MaxLeds; i++)
            {
        ...
        {
            SetKeys(0, (int)RazerChroma.Net.ChromaLink.Definitions.MaxLeds - 1, color);
        }
        ...
        public void Update()
        {
            Effect newEffect = _api.CreateChromaLinkEffect(rawEffect);
            newEffect.Set();
            lastEffect?.Delete();
